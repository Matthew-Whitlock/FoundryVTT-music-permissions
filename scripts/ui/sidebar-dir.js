import RemoteFolderConfig from "./remote-folder.js"
...
class SidebarDirectory extends SidebarTab {
   ...
   */
    ...
    if ( Settings.can_create() ) html.find('.create-folder').click(ev => this._onCreateFolder(ev));
    ...
    return [
      {
        ...
        condition: (header) => {
			...
			return Settings.can_edit(game.userId, folder)
		},
        callback: header => {
  ...
  _getEntryContextOptions() {
    return [
      ...
      },
      {
        ...
        condition: (li) => {
			const document = this.constructor.collection.get(li.data("documentId"))
			return Settings.can_edit(game.userId, document);
		},
        callback: li => {
          const document = this.constructor.collection.get(li.data("documentId"));
          if ( !document ) return;
